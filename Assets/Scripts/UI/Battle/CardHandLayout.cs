...
namespace Crookedile.UI.Battle
{
    ...
    public class CardHandLayout : MonoBehaviour
    {
        [Header("Arc Shape")]
        [Tooltip(
            "Degrees added per card stepping left-to-right. 4–7° feels natural. 0 = all upright."
        )]
        [SerializeField]
        private float angleStepDegrees = 5f;

        [Tooltip(
            "Radius of the imaginary circle cards sit on. "
                + "Controls how much edge cards dip below centre. Larger = flatter. Try 600–900."
        )]
        [SerializeField]
        private float arcRadius = 750f;

        [Tooltip("Maximum per-card tilt in degrees. Prevents extreme lean with a tiny hand.")]
        [SerializeField]
        private float maxAngleStepDegrees = 7f;

        [Header("Card Size")]
        [Tooltip(
            "Width of a single card in canvas pixels. Used to compute visible area per card when the hand is crowded."
        )]
        [SerializeField]
        ...
        [Header("Hover Spread")]
        [Tooltip(
            "Extra degrees added to the angle step when any card is hovered, "
                + "making the hand fan out slightly. 1–3° feels natural."
        )]
        [SerializeField]
        private float hoverSpreadAngleBonus = 2f;

        [Header("Debug Preview")]
        [Tooltip(
            "Draw ghost card outlines in the Scene view to preview arc layout without needing real cards in the hand."
        )]
        [SerializeField]
        private bool showDebugLayout = false;

        [Tooltip("Number of card slots to preview.")]
        [SerializeField, Range(1, 10)]
        private int debugPreviewCount = 5;

        [Tooltip(
            "Card height in canvas pixels used only for the ghost outline (width comes from Card Size above)."
        )]
        [SerializeField]
        private float debugCardHeight = 168f;

        [Tooltip("Outline colour for regular ghost card slots.")]
        [SerializeField]
        private Color debugCardColor = new Color(0.35f, 0.75f, 1f, 0.85f);

        [Tooltip("Outline colour for the centre card slot.")]
        [SerializeField]
        private Color debugCentreColor = new Color(1f, 0.9f, 0.2f, 0.95f);

        [Tooltip("Draw the arc curve the cards sit on.")]
        [SerializeField]
        private bool debugShowArc = true;

        [Tooltip("Colour of the arc curve.")]
        [SerializeField]
        private Color debugArcColor = new Color(1f, 1f, 1f, 0.2f);
        ...
        {
            if (_hoverSpreadActive == active && _hoverSpreadCard == hoveredCard)
                return;
        ...
        private void ApplyHoverSpread()
        {
            ...
            int pivotIdx =
                _hoverSpreadActive && _hoverSpreadCard != null
                    ? _cachedCards.IndexOf(_hoverSpreadCard)
                    : -1;

            for (int i = 0; i < count; i++)
            {
                if (_cachedCards[i] == null)
                    continue;
        ...
        #region Helpers
        private void StartLerpSequence(
            List<(CardButton btn, Vector3 targetPos, float targetAngle)> targets
        )
        {
            ...
            foreach (var (btn, pos, angle) in targets)
            {
                ...
                _lerpSequence.Join(
                    rt.DOLocalRotate(new Vector3(0f, 0f, -angle), lerpDuration, RotateMode.Fast)
        ...
        #region Arc Math (shared by ArrangeCards and OnDrawGizmos)
        /// <summary>
        ...
        private Vector3 ComputeLocalPosition(float angleDeg)
        {
            ...
            return new Vector3(arcRadius * Mathf.Sin(rad), arcRadius * (Mathf.Cos(rad) - 1f), 0f);
        }

        #endregion

        #region Scene-View Debug Preview
#if UNITY_EDITOR
        private void OnDrawGizmos()
        {
            if (!showDebugLayout || debugPreviewCount <= 0)
                return;
