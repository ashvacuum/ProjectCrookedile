...
namespace Crookedile.Gameplay.Battle
{
    ...
    public class HostilityGuardEffect : BattleEffect
    {
        ...
        [SerializeField] private HostilityCondition _condition = HostilityCondition.TargetIsHostile;
        ...
        public override string GetDescription()
        {
            ...
            string innerDesc = _effects.Count > 0
                ? string.Join(", ", _effects.Where(e => e != null).Select(e => e.GetDescription()))
                : "nothing";
        ...
        private static bool AnyLiving(EffectExecutionContext ctx, Func<EnemyController, bool> predicate)
        {
            if (ctx.AllEnemies == null) return false;
            foreach (var enemy in ctx.AllEnemies)
                if (!enemy.IsDefeated && predicate(enemy)) return true;
            return false;
        }

        private static bool AllLiving(EffectExecutionContext ctx, Func<EnemyController, bool> predicate)
        {
            if (ctx.AllEnemies == null || ctx.AllEnemies.Count == 0) return false;
            foreach (var enemy in ctx.AllEnemies)
                if (!enemy.IsDefeated && !predicate(enemy)) return false;
            return true;
        }
    }
}
